...
const chat_file_basename = process.argv[2]
const space_id = process.argv[3]
const chatAccessToken = process.argv[4]
...
ws.on('message', (buffer) => {
  fs.appendFile(`${chat_file_basename}.jsonl`, buffer + "\n", (err) => {
    if (err) throw err;
  })
  ...
  if (msg.kind == CHAT) {
    ...
    if (body.final) {
      ...
      fs.appendFile(`${chat_file_basename}.txt`, message + "\n", (err) => {
        if (err) throw err;
      })
      console.log(message)
    }
